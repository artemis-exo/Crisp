...
export default function Sidebar({ rooms, activeRoomId, onSelectRoom, onRoomsChange, wsConnected, messages, unread = {} }) {
  ...
  const handleProfilePicUpload = async (e) => {
    ...
    try {
      ...
      setMyStatus(data);
    } catch (_) { alert("Failed to upload profile picture"); }
    e.target.value = "";
  };
  ...
  return (
    <>
      <aside className="w-full lg:w-80 flex-shrink-0 bg-crisp-surface flex flex-col h-full border-r border-crisp-border overflow-hidden">
        ...
        <div className="px-4 pt-4 pb-3 border-b border-crisp-border"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative">
              <div className="w-10 h-10 bg-crisp-accent rounded-2xl flex items-center justify-center
                font-mono font-bold text-white text-base shadow-accent-sm flex-shrink-0">C</div>
              {totalUnread > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 bg-crisp-red rounded-full
                  flex items-center justify-center text-[8px] text-white font-bold animate-scaleIn">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5">
                <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 transition-colors
                  ${wsConnected ? "bg-crisp-green" : "bg-crisp-amber animate-pulse2"}`} />
                <span className="text-[11px] text-crisp-muted font-mono truncate">
                  {wsConnected ? "Connected" : "Connecting…"}
                </span>
              </div>
            </div>

            {/* Profile avatar — tap to edit profile */}
            <button onClick={() => setShowEditor(true)} title="Edit profile"
              ...
              <span className="absolute -bottom-0.5 -right-0.5 w-4 h-4 bg-crisp-surface border border-crisp-border
                rounded-full flex items-center justify-center">
                <svg className="w-2.5 h-2.5 text-crisp-muted" fill="none" viewBox="0 0 24 24"
            ...
            <button onClick={signOut}
              className="w-9 h-9 rounded-xl bg-crisp-card border border-crisp-border
                flex items-center justify-center text-crisp-muted active:scale-95 transition-all"
              title="Sign out">
          ...
          <div className="relative mt-3">
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-crisp-muted pointer-events-none"
              fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/>
            </svg>
            <input className="input-field pl-10 py-2.5 text-sm rounded-xl"
        ...
        {/* ── Stories ──────────────────────────────────────────────────────── */}
        <StoriesBar
          onOpenStories={(startUser, grouped) => {
            setStories(grouped);
            setStoryViewer(startUser);
          }}
        />
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 py-12">
              ...
              <div className="text-crisp-muted text-sm font-body">
        ...
        <div className="p-3 border-t border-crisp-border"
          style={{ paddingBottom: `max(12px, env(safe-area-inset-bottom))` }}>
          <button onClick={() => setShowModal(true)}
            className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl
              bg-crisp-accent/10 hover:bg-crisp-accent/20 active:bg-crisp-accent/25
              border border-crisp-accent/25 text-crisp-accent2 font-display font-semibold
      ...
      {showEditor && (
        <ProfileEditor onClose={() => {
          setShowEditor(false);
          // Refresh my status after editing
          getUserStatus(username).then(({ data }) => setMyStatus(data)).catch(() => {});
        }} />
      )}
...
function RoomItem({ room, active, onClick, username, lastMsg, unreadCount }) {
  ...
  const preview = lastMsg
    ? `${lastMsg.senderId === username ? "You: " : ""}${
        lastMsg.deletedForEveryone ? "🚫 Message deleted"
        : lastMsg.content || (lastMsg.mediaType ? "📎 Media" : "")
      }`
        ...
        <div className="flex items-center justify-between gap-2 mb-0.5">
          ...
          {lastMsg?.timestamp && (
            <span className={`text-[10px] flex-shrink-0 font-mono
              ${unreadCount > 0 ? "text-crisp-accent2" : "text-crisp-muted"}`}>
              {fmtTime(lastMsg.timestamp)}
            </span>
          )}
        </div>
        <div className="flex items-center justify-between gap-2">
          <span className={`text-xs truncate font-body
            ${unreadCount > 0 ? "text-crisp-sub" : "text-crisp-muted"}`}>
            {preview}
          </span>
          {unreadCount > 0 && (
            <span className="flex-shrink-0 min-w-[20px] h-5 px-1.5 rounded-full bg-crisp-accent
              text-white text-[10px] font-mono font-bold flex items-center justify-center animate-fadeIn">
...
function NewChatModal({ onClose, onCreated, myUsername, existingRooms }) {
  ...
  useEffect(() => {
    getAllUsers()
      .then(({ data }) => setUsers(data.filter((u) => u.username !== myUsername)))
  ...
  const handleCreate = async () => {
    ...
    try {
      const { data } = await createRoom({
        participantIds: [myUsername, ...selected],
        isGroup: type === "group",
      ...
      if (msg.includes("already exists")) {
        const ex = existingRooms.find((r) =>
          !(r.isGroup || r.group) && r.participantIds?.includes(selected[0])
        );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70 backdrop-blur-sm"
      ...
      <div className="bg-crisp-surface border border-crisp-border2 rounded-t-3xl sm:rounded-3xl
        w-full sm:max-w-sm shadow-2xl animate-slideBot sm:animate-scaleIn max-h-[92vh]
        flex flex-col overflow-hidden"
        style={{ paddingBottom: `max(16px, env(safe-area-inset-bottom))` }}>
        <div className="flex justify-center pt-3 pb-1 sm:hidden">
          <div className="w-10 h-1 bg-crisp-border2 rounded-full" />
        </div>
        <div className="flex items-center justify-between px-6 py-4">
          <h2 className="font-display font-bold text-lg text-crisp-text">New Chat</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-xl bg-crisp-card border border-crisp-border
            flex items-center justify-center text-crisp-muted text-xl active:scale-95">×</button>
        </div>
        <div className="px-6 pb-2 flex-1 overflow-y-auto">
            ...
            {[["dm","Direct"],["group","Group"]].map(([t, l]) => (
              <button key={t} onClick={() => { setType(t); setSelected([]); }}
                className={`flex-1 py-2.5 rounded-xl font-display font-semibold text-sm transition-all
                  ${type === t ? "bg-crisp-accent text-white" : "text-crisp-muted"}`}>{l}</button>
            ))}
          </div>
          {type === "group" && (
            <div className="mb-5">
          ...
          <div className="bg-crisp-bg border border-crisp-border rounded-2xl overflow-hidden max-h-52 overflow-y-auto mb-4">
            {usersLoading ? (
              <div className="py-8 text-center text-crisp-muted text-sm">Loading…</div>
            ) : users.length === 0 ? (
              <div className="py-8 text-center text-crisp-muted text-sm px-4">No other users found.</div>
            ) : (
              users.map((u) => {
                const sel = selected.includes(u.username);
                return (
                  <div key={u.username} onClick={() => toggle(u.username)}
                    className={`flex items-center gap-3 px-4 py-3.5 cursor-pointer active:bg-crisp-hover
                      border-b border-crisp-border/40 last:border-0 ${sel ? "bg-crisp-accent/10" : ""}`}>
                    ...
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs flex-shrink-0
                      ${sel ? "bg-crisp-accent border-crisp-accent text-white" : "border-crisp-border2"}`}>
                      {sel && "✓"}
                    </div>
                  </div>
                );
          ...
          {error && (
            <div className="mb-4 text-red-300 text-sm bg-red-500/10 border border-red-500/20 px-4 py-3 rounded-xl">{error}</div>
          )}
        </div>
